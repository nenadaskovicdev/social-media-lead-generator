...
class TikTokScraper:
    def __init__(self):
        ...
        self.brightdata_tik_tok_tag_api_key = os.getenv(
            "BRIGHTDATA_TIK_TOK_TAG_API_KEY"
        )
        ...
        self.db = self.client.tiktok_scraper
        ...
        # Initialize UI
        self.root = tk.Tk()
        self.root.title("TikTok Profile Scraper")
    ...
    def setup_ui(self):
        ...
        urls = [
            ...
            "https://www.tiktok.com/@newyorkcityguide/video/7396463220493471018",
            "https://www.tiktok.com/@1twobacalmyshoe/video/7467375443495226655",
            "https://www.tiktok.com/@landmarksofny",
            "https://www.tiktok.com/@jeffreyinnyc/video/7540307064220847415",
            "https://www.tiktok.com/@followme_nyc/video/7466595427840724255",
            "https://www.tiktok.com/@yankees",
            "https://www.tiktok.com/@newyork_cityvibes/video/7461752216924343582",
            "https://www.tiktok.com/@im_billie7/video/7537377752857480462",
            "https://www.tiktok.com/@nilsdecks/video/7397515438365625632",
            "https://www.tiktok.com/@nytoanywhere/video/7245079310124453163",
            "https://www.tiktok.com/@xoxoemira/video/7120005555753749806",
            "https://www.tiktok.com/@c4news/video/7532572653727517974",
            "https://www.tiktok.com/@thekatieromero",
        ]
    ...
    def scrape_profiles(self, usernames):
        ...
        try:
            ...
            data = [
                {
                    "search_url": f"https://www.tiktok.com/search?q={username}&lang=en",
                    "country": "US",
                }
                for username in usernames
            ]
            ...
            if response.status_code != 200:
                ...
                try:
                    self.log_message(f"Response content: {response.json()}")
                except Exception:
                    self.log_message(f"Response content (raw): {response.text}")
                return
    ...
    def extract_accounts(self, data):
        """Extract account info from raw TikTok snapshot data safely"""
        EMAIL_RE = re.compile(
        ...
        def find_mentions(obj):
            return set(MENTION_RE.findall(json.dumps(obj)))

        def find_instagram_usernames(obj):
            return set(
                m.group(1) for m in INSTAGRAM_URL_RE.finditer(json.dumps(obj))
            )
        ...
        for item in data if isinstance(data, list) else [data]:
            ...
            username = item.get("account_id")
            profile_url = item.get("url")
            bio = item.get("biography") or item.get("signature") or ""
            followers = item.get("followers", 0)
            full_name = item.get("nickname")
            ...
            # Also parse top videos for extra info
            for video in item.get("top_videos", []):
                emails |= find_emails(video)
                mentions |= find_mentions(video)
                inst_users |= find_instagram_usernames(video)
                links |= gather_links(video)

            # Also parse top posts for extra info
            for post in item.get("top_posts_data", []):
                emails |= find_emails(post)
                mentions |= find_mentions(post)
                inst_users |= find_instagram_usernames(post)
            ...
            extracted.append(
                {
                    ...
                    "extra_usernames": sorted(mentions | inst_users),
